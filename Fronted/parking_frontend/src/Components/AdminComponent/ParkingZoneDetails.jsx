...
const ParkingZoneDetails = () => {
  ...
  useEffect(()=>{
    const getZone = async() => {
        try{
            const response = await Axios.get("http://localhost:7070/parkingzone/zone", 
            {
              headers : {
                'Authorization' : `Bearer ${token}`
              }
...
const handleSave = async () => {
  ...
  await Axios.put(`http://localhost:7070/parkingzone/zone/${editedZone.zoneId}`, editedZone, {headers:headers});
